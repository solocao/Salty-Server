...
class NOAA {
  ...
  fetchHourlyPredictions(stationId) {
    const params =
      ...
      '&end_date=' +
      endDate +
      '&product=predictions&interval=hilo&datum=mllw&units=english&time_zone=gmt&application=web_services&format=json'
  ...
  groupByDay(predictions) {
    return _.groupBy(predictions, prediction => {
      return prediction.time.format(API_DATE_FORMAT)
    })
  }
}
