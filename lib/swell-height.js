...
class SwellHeight {
  fetchSwellHeights(latitude, longitude) {
    return this.findNearestSwell(longitude, latitude)
      .then(spotId => this._fetchRawSwellData(spotId))
      .then(forecast => this._formatSwellData(forecast))
      .then(formattedSwellData => {
        return formattedSwellData;
      })
      .catch(error => {
        console.log('Error fetching swell data', error)
      })
  }

  findNearestSwell(longitude, latitude) {
